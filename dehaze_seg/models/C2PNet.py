...
class ConvBNReLU(nn.Module):
    def __init__(self, in_ch, out_ch, k=3, s=1, p=1, use_bn=True):
        super().__init__()
        padding_mode = 'reflect' if p > 0 else 'zeros'
        layers = [nn.Conv2d(in_ch, out_ch, k, s, p, bias=not use_bn, padding_mode=padding_mode)]
        if use_bn:
...
class SpatialAttention(nn.Module):
    def __init__(self, in_ch):
        super().__init__()
        self.conv = nn.Conv2d(in_ch, 1, kernel_size=7, padding=3, padding_mode='reflect')
...
class C2PNet(nn.Module):
    ...
    def __init__(self, in_ch=3, base_ch=32, blocks_per_group=6, groups=3, use_pdu=True, gain_scale=0.3):
        ...
        self.coarse_head = nn.Sequential(
            ...
            nn.Conv2d(ch3//2, in_ch, kernel_size=3, padding=1, padding_mode='reflect'),
            nn.Sigmoid()
        )
        ...
        self.refine = nn.Sequential(
            ...
            nn.Conv2d(max(64, ch1), in_ch, kernel_size=3, padding=1, padding_mode='reflect'),
    ...
    def load_state_dict(self, state_dict, strict=True):
        # Compatibility for checkpoints produced by the temporary ReflectConv2d
        # wrapper, whose parameters were saved as "*.conv.weight/bias".
        mapped = {}
        for key, value in state_dict.items():
            new_key = key
            if ".net.0.conv." in new_key:
                new_key = new_key.replace(".net.0.conv.", ".net.0.")
            if new_key.endswith(".sa.conv.conv.weight"):
                new_key = new_key.replace(".sa.conv.conv.weight", ".sa.conv.weight")
            elif new_key.endswith(".sa.conv.conv.bias"):
                new_key = new_key.replace(".sa.conv.conv.bias", ".sa.conv.bias")
            if new_key.startswith("coarse_head.1.conv."):
                new_key = new_key.replace("coarse_head.1.conv.", "coarse_head.1.")
            if new_key.startswith("refine.2.conv."):
                new_key = new_key.replace("refine.2.conv.", "refine.2.")
