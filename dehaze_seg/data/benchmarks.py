...
def nh_clear_stem_from_hazy(stem: str) -> str:
    if stem.endswith("_hazy"):
        return stem[:-5] + "_GT"
    return stem + "_GT"


def find_nh_clear(clear_dir: Path, hazy_stem: str) -> Optional[Path]:
    candidates = [
        nh_clear_stem_from_hazy(hazy_stem),
        hazy_stem.replace("_hazy", "_GT"),
        hazy_stem.replace("hazy", "GT"),
        hazy_stem,
    ]
    base = hazy_stem.split("_")[0]
    candidates.extend([base + "_GT", base])
    for stem in dict.fromkeys(candidates):
        p = find_by_stem(clear_dir, stem)
        if p is not None:
            return p
    return None
...
class NHHAZEDataset(Dataset):
    def __init__(
        ...
        if ids is None:
            ids = []
            for p in self.hazy_dir.iterdir():
                if p.is_file() and p.suffix.lower() in IMG_EXTS and find_nh_clear(self.clear_dir, p.stem) is not None:
                    ids.append(p.stem)
            ids.sort()
        self.ids = list(ids)
        if not self.ids:
            raise RuntimeError(f"No matched NH-HAZE hazy/clear samples found under {self.root}")
    ...
    def _sync_transform(self, hazy: Image.Image, clear: Image.Image):
        ...
        if self.augment:
            ...
            if random.random() < 0.25:
                hazy = TF.adjust_brightness(hazy, random.uniform(0.94, 1.06))
                hazy = TF.adjust_contrast(hazy, random.uniform(0.94, 1.06))
        return hazy, clear

    def __getitem__(self, index: int):
        stem = self.ids[index]
        hp = find_by_stem(self.hazy_dir, stem)
        cp = find_nh_clear(self.clear_dir, stem)
        if hp is None or cp is None:
            raise RuntimeError(f"Missing pair for {stem}: hazy={hp}, clear={cp}")
        hazy = Image.open(hp).convert("RGB")
        clear = Image.open(cp).convert("RGB")
