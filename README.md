...
Sample `003` is the first ID in the supplied historical checkpoints' validation split. For other checkpoints, omit `--sample` to use their first validation ID, or select an ID from their saved split. Training samples and known reference overlap are rejected by default. For an explicitly requested qualitative training illustration, add `--split train --sample 060 --allow-training-overlap`; its caption and JSON identify it as a training-data diagnostic. Use `--native-resolution` in place of `--resize 512 512` to infer directly on the original image grid, matching the original exporter. Native and resized scores use different grids and must not be mixed. This is a single validation example, not an independent test set or a reproduction of Table 1.

Scores are **mIoU / mDice**, computed from the same full hard masks at the selected inference resolution, including background and foreground. They are generated directly from the metric records. The figure places scores above the full-image row, matching red-box crops below it, and model names at the bottom. The enlargements do not change the scoring region. Display panels preserve the original aspect ratio. `--roi X0 Y0 X1 Y1` sets one normalized crop for every panel (default: `0.30 0.48 0.70 0.77`). All prediction/target masks, confusion matrices, checkpoint/input hashes, CSV/JSON records and the caption accompany `figure7.png` (300 dpi) and `figure7.pdf`.

Panel titles show model names only. Titles, metric numbers and `mIoU / mDice` use **Times New Roman**. If the font is not installed (for example on Linux), pass `--metric-font /path/to/timesbd.ttf` for the bold paper layout; missing fonts produce an explicit error.
